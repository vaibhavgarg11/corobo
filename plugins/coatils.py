...
class Coatils(BotPlugin):
    ...
    def run(self, msg, match):
        ...
        """
        lang = match.group(1)
        bear_settings = Coatils.construct_settings(match.group(2))
        code = match.group(3)
        ...
        try:
            ...
            yield ('Here is what I think is wrong: '
                   if results else 'Your code is flawless :tada:')
            for result in results:
                ...
                diff_message = ''
                if diffs:
                    diff_message += 'These patches can help solve the issue: \n'
                    for diff in diffs:
                        diff = diff.splitlines(True)[2:]
                        extra_indented_diff = ''.join(
                            map(lambda x: '   ' + x, diff)
                        )
                        diff_message += '```diff\n{}\n```\n'.format(
                            extra_indented_diff.strip()
                        )

                yield '- {} - {} :\n{}{diff}'.format(origin, afm, message,
                                                     diff=diff_message)
